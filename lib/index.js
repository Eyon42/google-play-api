...
router.get("/apps/:appId", function (req, res, next) {
  ...
  gplay.app(opts).then(cleanUrls(req)).then(res.json.bind(res)).catch(next);
});
...
router.get("/apps/:appId/datasafety", function (req, res, next) {
  const opts = Object.assign({ appId: req.params.appId }, req.query);
  gplay.datasafety(opts).then(toList).then(res.json.bind(res)).catch(next);
});

/* App permissions */
router.get("/apps/:appId/permissions", function (req, res, next) {
  const opts = Object.assign({ appId: req.params.appId }, req.query);
  gplay.permissions(opts).then(toList).then(res.json.bind(res)).catch(next);
});
...
router.get("/categories/", function (req, res, next) {
  gplay.categories().then(res.json.bind(res)).catch(next);
});
